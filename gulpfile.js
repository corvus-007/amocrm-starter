...
gulp.task('manifest', ['js', 'i18n', 'css', 'img', 'templates'], function() {
  return gulp
    ...
    .pipe(
      jmodify({
        key: 'widget.version',
        value: semver.inc(manifest.widget.version, 'patch', null)
      })
    )
...
gulp.task('build', ['zip'], function() {
  gutil.log(
    gutil.colors.green('Successfull build: '),
    gutil.colors.cyan(path.publish + zipFileName)
  );
});
